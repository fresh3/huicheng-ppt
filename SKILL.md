...
**内容页扩展色（从真实年报蒸馏，供内容元素选用）：**

| 用途 | 色值 | 说明 |
|------|------|------|
| 数据红 | `#FF0000` | 数据分析结论、关键数字、增减趋势标注 |
| 正文深灰 | `#404040` | 表格正文、分析段落（比纯黑柔和） |
| 辅助灰 | `#595959` | 图表标签、次级说明 |
| 浅灰文字 | `#646464` | 脚注、表格次级数据 |
| 深蓝 | `#0070C0` | 图表柱状/折线辅助色 |
| 亮蓝 | `#2196F3` / `#08AEEA` | 图表高亮、进度环 |
| 浅绿图表 | `#4AB258` | 图表数据系列辅助绿 |
| 表头黑 | `#000000` | 表格表头背景填充 |
| 斑马行 | `#F2F2F2` | 表格交替行背景 |
| 边框灰 | `#ABABAB` | 表格次级分隔线 |
...
| 元素 | 字号(pt) | 粗体 | 颜色 |
|------|---------|------|------|
| 表格正文 | 11-12 | 否 | #404040 或 #181715 |
| 表头文字 | 11 | 是 | #FFFFFF（黑/深灰底上白字） |
| 图表标签 | 9-11 | 否 | #595959 / #646464 |
| 脚注/数据来源 | 9-10 | 否 | #404040 |
| 分析结论段落 | 14-16 | 否 | #404040；关键数字用 #FF0000 |

## 内容页表格样式（从真实年报蒸馏）

内容页表格有两种风格，按场景选用：

**风格A — 黑表头（2024年报风格，推荐数据密集页）：**

| 区域 | 背景填充 | 文字颜色 | 字号 | 粗体 |
|------|---------|---------|------|------|
| 表头行 | `#000000` | `#FFFFFF` | 11pt | 是 |
| 数据行（奇数） | `#FFFFFF` | `#000000` | 10pt | 否 |
| 数据行（偶数） | `#F2F2F2` | `#000000` | 10pt | 否 |
| 强调数据 | `#FF0000` 填充或文字 | — | 10pt | 否 |

**风格B — 深灰表头（2021年报风格，柔和商务感）：**

| 区域 | 背景填充 | 文字颜色 | 字号 | 粗体 |
|------|---------|---------|------|------|
| 表头行 | `#404040` | `#FFFFFF` | 11pt | 是 |
| 数据行（奇数） | `#FFFFFF` | `#404040` | 10-11pt | 否 |
| 数据行（偶数） | `#F2F2F2` | `#404040` | 10-11pt | 否 |

**表格通用规则：**
- 最大宽度 `11.5in`（左右边距约 0.94in）
- 行高：表头 0.4in，数据行 0.3in
- 数字列右对齐用 Arial，文字列左对齐用微软雅黑

## 内容页分析结论框（从真实年报蒸馏）

年报内容页典型结构：主内容区（图表/表格）+ 底部分析框，分析框规律如下：

- **位置**：y=5.0-6.3in，宽度 10-11in，紧贴底栏上方
- **开头**："数据观察："或"分析结论："加粗
- **正文**：#404040，不加粗，14-16pt
- **关键数字/趋势**：用 #FF0000 标注（如"增长39%"、"下降200万"）
- **换行**：多条观察用"1、... 2、..."编号，或换行分段
